...
public:
	...
	bool checkResultSteamID(uint64_t steam_id_expected, int32 this_inventory_handle);
	...
	String getResultItemProperty(uint32 index, String name, int32 this_inventory_handle);
	Array getResultItems(int32 this_inventory_handle);
	int getResultStatus(int32 this_inventory_handle);
	uint32 getResultTimestamp(int32 this_inventory_handle);
...
//		Dictionary receivedRelayAuthTicket();	<------ Uses datagram relay structs which were removed from base SDK
	void resetIdentity(uint64_t remote_stea_id);
